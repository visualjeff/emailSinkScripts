...
function helpMsg {
        echo ""
        echo "** usage:   ./update_server_only.sh IMAGE_FILE IMAGE_NAME IMAGE_VERSION"
        echo "** example: ./update_server_only.sh haraka-couchdb.tar visualjeff/haraka-couchdb 1.0.0"
    echo ""
	}
## END HELPERS
...
if [ ! -f $image_file ]; then
  ...
  echo ""
  echo "Now finished the update by re-importing your data:  ./finishUpdate.sh"
fi
